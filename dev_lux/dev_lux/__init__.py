...
@app.route('/sc/<language>', methods=['POST', 'GET'])
def setcookie(language):
...
def homepage():
    ...
    c.execute('SELECT * FROM lux_members WHERE honor="standard" LIMIT 4')
    list_membres_standard = c.fetchall()

    list_membres_standard_1 = []
    for elem in list_membres_standard[0]:
        try:
            list_membres_standard_1.append(elem.decode("utf8"))
        except AttributeError:
            list_membres_standard_1.append(elem)

    list_membres_standard_2 = []
    for elem in list_membres_standard[1]:
        try:
            list_membres_standard_2.append(elem.decode("utf8"))
        except AttributeError:
            list_membres_standard_2.append(elem)

    list_membres_standard_3 = []
    for elem in list_membres_standard[2]:
        try:
            list_membres_standard_3.append(elem.decode("utf8"))
        except AttributeError:
            list_membres_standard_3.append(elem)

    list_membres_standard_4 = []
    for elem in list_membres_standard[3]:
        try:
            list_membres_standard_4.append(elem.decode("utf8"))
        except AttributeError:
            list_membres_standard_4.append(elem)

    c.execute('SELECT * FROM lux_members WHERE honor="honored" LIMIT 5')
    list_membres_honored = c.fetchall()

    list_membres_honored_1 = []
    for elem in list_membres_honored[0]:
        try:
            list_membres_honored_1.append(elem.decode("utf8"))
        except AttributeError:
            list_membres_honored_1.append(elem)

    list_membres_honored_2 = []
    for elem in list_membres_honored[1]:
        try:
            list_membres_honored_2.append(elem.decode("utf8"))
        except AttributeError:
            list_membres_honored_2.append(elem)

    list_membres_honored_3 = []
    for elem in list_membres_honored[2]:
        try:
            list_membres_honored_3.append(elem.decode("utf8"))
        except AttributeError:
            list_membres_honored_3.append(elem)

    list_membres_honored_4 = []
    for elem in list_membres_honored[3]:
        try:
            list_membres_honored_4.append(elem.decode("utf8"))
        except AttributeError:
            list_membres_honored_4.append(elem)

    list_membres_honored_5 = []
    for elem in list_membres_honored[4]:
        try:
            list_membres_honored_5.append(elem.decode("utf8"))
        except AttributeError:
            list_membres_honored_5.append(elem)
    ...
    if language == "fr":
        c.execute('SELECT txt_fr_1 FROM lux_content WHERE page="homepage";')
        content_homepage = c.fetchone()[0].decode("utf8")
    elif language == "en":
        c.execute('SELECT txt_en_1 FROM lux_content WHERE page="homepage";')
        content_homepage = c.fetchone()[0].decode("utf8")
    elif language == "lux":
        c.execute('SELECT txt_lux_1 FROM lux_content WHERE page="homepage";')
        content_homepage = c.fetchone()[0].decode("utf8")
    elif language == "all":
        c.execute('SELECT txt_all_1 FROM lux_content WHERE page="homepage";')
        content_homepage = c.fetchone()[0].decode("utf8")
    else:
        c.execute('SELECT txt_fr_1 FROM lux_content WHERE page="homepage";')
        content_homepage = c.fetchone()[0].decode("utf8")
...
def decouvrir():
    ...
    if language == "fr":
        c.execute('SELECT txt_fr_1, txt_fr_2, txt_fr_3, txt_fr_4 FROM lux_content WHERE page="decouvrir";')
        content_decouvrir = [x.decode('utf8') for x in c.fetchall()[0]]
    elif language == "en":
        c.execute('SELECT txt_en_1, txt_en_2, txt_en_3, txt_en_4 FROM lux_content WHERE page="decouvrir";')
        content_decouvrir = [x.decode('utf8') for x in c.fetchall()[0]]
    elif language == "lux":
        c.execute('SELECT txt_lux_1, txt_lux_2, txt_lux_3, txt_lux_4 FROM lux_content WHERE page="decouvrir";')
        content_decouvrir = [x.decode('utf8') for x in c.fetchall()[0]]
    elif language == "all":
        c.execute('SELECT txt_all_1, txt_all_2, txt_all_3, txt_all_4 FROM lux_content WHERE page="decouvrir";')
        content_decouvrir = [x.decode('utf8') for x in c.fetchall()[0]]
    else:
        c.execute('SELECT txt_fr_1, txt_fr_2, txt_fr_3, txt_fr_4 FROM lux_content WHERE page="decouvrir";')
        content_decouvrir = [x.decode('utf8') for x in c.fetchall()[0]]

    return render_template('decouvrir.html', content_decouvrir=content_decouvrir)
...
def investir():
    ...
    if language == "fr":
        c.execute('SELECT txt_fr_1, txt_fr_2, txt_fr_3, txt_fr_4, txt_fr_5 FROM lux_content WHERE page="investir";')
        content_investir = [x.decode('utf8') for x in c.fetchall()[0]]
    elif language == "en":
        c.execute('SELECT txt_en_1, txt_en_2, txt_en_3, txt_en_4, txt_en_5 FROM lux_content WHERE page="investir";')
        content_investir = [x.decode('utf8') for x in c.fetchall()[0]]
    elif language == "lux":
        c.execute('SELECT txt_lux_1, txt_lux_2, txt_lux_3, txt_lux_4, txt_lux_5 FROM lux_content WHERE page="investir";')
        content_investir = [x.decode('utf8') for x in c.fetchall()[0]]
    elif language == "all":
        c.execute('SELECT txt_all_1, txt_all_2, txt_all_3, txt_all_4, txt_all_5 FROM lux_content WHERE page="investir";')
        content_investir = [x.decode('utf8') for x in c.fetchall()[0]]
    else:
        c.execute('SELECT txt_fr_1, txt_fr_2, txt_fr_3, txt_fr_4, txt_fr_5 FROM lux_content WHERE page="investir";')
        content_investir = [x.decode('utf8') for x in c.fetchall()[0]]

    return render_template('investir.html', content_investir=content_investir)
...
def membres():
    ...
    list_membres_standard = list(list_membres_standard)
    for elem in range(0, len(list_membres_standard)):
        list_membres_standard[elem] = list(list_membres_standard[elem])

    for i in range(0, len(list_membres_standard)):
        for x in range(0, len(list_membres_standard[i])):
            try:
                list_membres_standard[i][x] = list_membres_standard[i][x].decode("utf8")
            except AttributeError:
                pass
    ...
    list_membres_honored = list(list_membres_honored)
    for elem in range (0, len(list_membres_honored)):
        list_membres_honored[elem] = list(list_membres_honored[elem])

    for i in range (0, len(list_membres_honored)):
        for x in range (0, len(list_membres_honored[i])):
            try:
                list_membres_honored[i][x] = list_membres_honored[i][x].decode("utf8")
            except AttributeError:
                pass
    ...
    if language == "fr":
        c.execute('SELECT txt_fr_1, txt_fr_2, txt_fr_3, txt_fr_4, txt_fr_5, txt_fr_6, txt_fr_7, txt_fr_8, txt_fr_9 FROM lux_content WHERE page="members";')
        content_membres = [x.decode('utf8') for x in c.fetchall()[0]]
    elif language == "en":
        c.execute('SELECT txt_en_1, txt_en_2, txt_en_3, txt_en_4, txt_en_5, txt_en_6, txt_en_7, txt_en_8, txt_en_9 FROM lux_content WHERE page="members";')
        content_membres = [x.decode('utf8') for x in c.fetchall()[0]]
    elif language == "lux":
        c.execute('SELECT txt_lux_1, txt_lux_2, txt_lux_3, txt_lux_4, txt_lux_5, txt_lux_6, txt_lux_7, txt_lux_8, txt_lux_9 FROM lux_content WHERE page="members";')
        content_membres = [x.decode('utf8') for x in c.fetchall()[0]]
    elif language == "all":
        c.execute('SELECT txt_all_1, txt_all_2, txt_all_3, txt_all_4, txt_all_5, txt_all_6, txt_all_7, txt_all_8, txt_all_9 FROM lux_content WHERE page="members";')
        content_membres = [x.decode('utf8') for x in c.fetchall()[0]]
    else:
        c.execute('SELECT txt_fr_1, txt_fr_2, txt_fr_3, txt_fr_4, txt_fr_5, txt_fr_6, txt_fr_7, txt_fr_8, txt_fr_9 FROM lux_content WHERE page="members";')
        content_membres = [x.decode('utf8') for x in c.fetchall()[0]]

    return render_template('membres.html', **locals())
...
def shopping():
    ...
    if language == "fr":
        c.execute('SELECT txt_fr_1, txt_fr_2, txt_fr_3, txt_fr_4, txt_fr_5 FROM lux_content WHERE page="shopping";')
        content_shopping = [x.decode('utf8') for x in c.fetchall()[0]]
    elif language == "en":
        c.execute('SELECT txt_en_1, txt_en_2, txt_en_3, txt_en_4, txt_en_5 FROM lux_content WHERE page="shopping";')
        content_shopping = [x.decode('utf8') for x in c.fetchall()[0]]
    elif language == "lux":
        c.execute('SELECT txt_lux_1, txt_lux_2, txt_lux_3, txt_lux_4, txt_lux_5 FROM lux_content WHERE page="shopping";')
        content_shopping = [x.decode('utf8') for x in c.fetchall()[0]]
    elif language == "all":
        c.execute('SELECT txt_all_1, txt_all_2, txt_all_3, txt_all_4, txt_all_5 FROM lux_content WHERE page="shopping";')
        content_shopping = [x.decode('utf8') for x in c.fetchall()[0]]
    else:
        c.execute('SELECT txt_fr_1, txt_fr_2, txt_fr_3, txt_fr_4, txt_fr_5 FROM lux_content WHERE page="shopping";')
        content_shopping = [x.decode('utf8') for x in c.fetchall()[0]]

    return render_template('shopping.html', content_shopping=content_shopping)
...
def admin_publications():
    if request.method == 'POST':
        if "formMembers" in request.form :
            ...
            logo_hotel = request.form.get('logoHotel')

            if member_honor == "standard":
                f = request.files['file']
                fullDirFile = '/var/www/dev_lux/dev_lux/static/uploaded_pics/' + secure_filename(f.filename)
                f.save(fullDirFile)

                c.execute(
                    'INSERT INTO lux_members (member_name, member_surname, member_status, member_etoiles, member_addr, member_img, honor, logoHotel) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)',
                    (member_name, member_surname, member_status, member_etoiles, member_adresse,
                     secure_filename(f.filename), member_honor, logo_hotel))
                conn.commit()
            else:
                c.execute(
                    'INSERT INTO lux_members (member_name, member_surname, member_status, member_etoiles, member_addr, honor, logoHotel) VALUES (%s, %s, %s, %s, %s, %s, %s)',
                    (member_name, member_surname, member_status, member_etoiles, member_adresse, member_honor, logo_hotel))
                conn.commit()
            ...
            c.execute("INSERT INTO lux_events (event_date, event_description, event_image) VALUES (%s, %s, %s)", (event_date, event_description, secure_filename(f.filename)))
            conn.commit()
            ...
            c.execute("INSERT INTO lux_annonces (titre, contenu, image) VALUES (%s, %s, %s)", (contenu, titre_publication, secure_filename(f.filename)))
            conn.commit()
...
def admin_langues():
    if request.method == 'POST':
        ...
        content_homepage_fr = txt5[1].decode("utf8")
        content_homepage_en = txt5[10].decode("utf8")
        content_homepage_lux = txt5[19].decode("utf8")
        content_homepage_all = txt5[28].decode("utf8")

        return render_template("accesadmin/langues.html", **locals())
        # return (content_homepage_fr + "#####" + content_homepage_en + "#####" + content_homepage_lux + "#####" + content_homepage_all)
...
def admin_pub():
    if request.method == 'POST':  # event
        ...
        c.execute('INSERT INTO lux_pub (category, contenu, image, url) VALUES (%s, %s, %s, %s)',
                  (category, contenu, secure_filename(f.filename), url))
        conn.commit()
...
@app.route('/cdl/langues/', methods=['GET', 'POST'])
@login_required
def cdlLangues():
    if request.method == 'POST':
        if "form_trad_membres" in request.form:
            ...
            c.execute(
                'UPDATE lux_content SET txt_fr_1 = %s, txt_fr_2 = %s, txt_fr_3 = %s, txt_fr_4 = %s, txt_fr_5 = %s, txt_fr_6 = %s, txt_fr_7 = %s, txt_fr_8 = %s, txt_fr_9 = %s WHERE page="members"',
                (txt_fr_1, txt_fr_2, txt_fr_3, txt_fr_4, txt_fr_5, txt_fr_6, txt_fr_7, txt_fr_8, txt_fr_9))
            conn.commit()

            c.execute(
                'UPDATE lux_content SET txt_en_1 = %s, txt_en_2 = %s, txt_en_3 = %s, txt_en_4 = %s, txt_en_5 = %s, txt_en_6 = %s, txt_en_7 = %s, txt_en_8 = %s, txt_en_9 = %s WHERE page="members"',
                (txt_en_1, txt_en_2, txt_en_3, txt_en_4, txt_en_5, txt_en_6, txt_en_7, txt_en_8, txt_en_9))
            conn.commit()

            c.execute(
                'UPDATE lux_content SET txt_lux_1 = %s, txt_lux_2 = %s, txt_lux_3 = %s, txt_lux_4 = %s, txt_lux_5 = %s, txt_lux_6 = %s, txt_lux_7 = %s, txt_lux_8 = %s, txt_lux_9 = %s WHERE page="members"',
                (txt_lux_1, txt_lux_2, txt_lux_3, txt_lux_4, txt_lux_5, txt_lux_6, txt_lux_7, txt_lux_8, txt_lux_9))
            conn.commit()

            c.execute(
                'UPDATE lux_content SET txt_all_1 = %s, txt_all_2 = %s, txt_all_3 = %s, txt_all_4 = %s, txt_all_5 = %s, txt_all_6 = %s, txt_all_7 = %s, txt_all_8 = %s, txt_all_9 = %s WHERE page="members"',
                (txt_all_1, txt_all_2, txt_all_3, txt_all_4, txt_all_5, txt_all_6, txt_all_7, txt_all_8, txt_all_9))
            conn.commit()

            return redirect(url_for('cdlLangues'))
            ...
            c.execute(
                'UPDATE lux_content SET txt_fr_1 = %s, txt_fr_2 = %s, txt_fr_3 = %s, txt_fr_4 = %s, txt_fr_5 = %s WHERE page="investir"',
                (txt_fr_1, txt_fr_2, txt_fr_3, txt_fr_4, txt_fr_5))
            conn.commit()

            c.execute(
                'UPDATE lux_content SET txt_en_1 = %s, txt_en_2 = %s, txt_en_3 = %s, txt_en_4 = %s, txt_en_5 = %s WHERE page="investir"',
                (txt_en_1, txt_en_2, txt_en_3, txt_en_4, txt_en_5))
            conn.commit()

            c.execute(
                'UPDATE lux_content SET txt_lux_1 = %s, txt_lux_2 = %s, txt_lux_3 = %s, txt_lux_4 = %s, txt_lux_5 = %s WHERE page="investir"',
                (txt_lux_1, txt_lux_2, txt_lux_3, txt_lux_4, txt_lux_5))
            conn.commit()

            c.execute(
                'UPDATE lux_content SET txt_all_1 = %s, txt_all_2 = %s, txt_all_3 = %s, txt_all_4 = %s, txt_all_5 = %s WHERE page="investir"',
                (txt_all_1, txt_all_2, txt_all_3, txt_all_4, txt_all_5))
            conn.commit()
            ...
            c.execute(
                'UPDATE lux_content SET txt_fr_1 = %s, txt_fr_2 = %s, txt_fr_3 = %s, txt_fr_4 = %s WHERE page="decouvrir"',
                (txt_fr_1, txt_fr_2, txt_fr_3, txt_fr_4))
            conn.commit()

            c.execute(
                'UPDATE lux_content SET txt_en_1 = %s, txt_en_2 = %s, txt_en_3 = %s, txt_en_4 = %s WHERE page="decouvrir"',
                (txt_en_1, txt_en_2, txt_en_3, txt_en_4))
            conn.commit()

            c.execute(
                'UPDATE lux_content SET txt_lux_1 = %s, txt_lux_2 = %s, txt_lux_3 = %s, txt_lux_4 = %s WHERE page="decouvrir"',
                (txt_lux_1, txt_lux_2, txt_lux_3, txt_lux_4))
            conn.commit()

            c.execute(
                'UPDATE lux_content SET txt_all_1 = %s, txt_all_2 = %s, txt_all_3 = %s, txt_all_4 = %s WHERE page="decouvrir"',
                (txt_all_1, txt_all_2, txt_all_3, txt_all_4))
            conn.commit()
            ...
            c.execute(
                'UPDATE lux_content SET txt_fr_1 = %s, txt_fr_2 = %s, txt_fr_3 = %s, txt_fr_4 = %s, txt_fr_5 = %s WHERE page="shopping"',
                (txt_fr_1, txt_fr_2, txt_fr_3, txt_fr_4, txt_fr_5))
            conn.commit()

            c.execute(
                'UPDATE lux_content SET txt_en_1 = %s, txt_en_2 = %s, txt_en_3 = %s, txt_en_4 = %s, txt_en_5 = %s WHERE page="shopping"',
                (txt_en_1, txt_en_2, txt_en_3, txt_en_4, txt_en_5))
            conn.commit()

            c.execute(
                'UPDATE lux_content SET txt_lux_1 = %s, txt_lux_2 = %s, txt_lux_3 = %s, txt_lux_4 = %s, txt_lux_5 = %s WHERE page="shopping"',
                (txt_lux_1, txt_lux_2, txt_lux_3, txt_lux_4, txt_lux_5))
            conn.commit()

            c.execute(
                'UPDATE lux_content SET txt_all_1 = %s, txt_all_2 = %s, txt_all_3 = %s, txt_all_4 = %s, txt_all_5 = %s WHERE page="shopping"',
                (txt_all_1, txt_all_2, txt_all_3, txt_all_4, txt_all_5))
            conn.commit()
            ...
            c.execute(
                'UPDATE lux_content SET txt_fr_1 = %s, txt_en_1 = %s, txt_lux_1 = %s, txt_all_1 = %s WHERE page="homepage"',
                (hp_fr, hp_en, hp_lux, hp_all))
            conn.commit()
        ...
        return render_template("accescdl/langues.html", **locals())

@app.route('/cdl/publications/', methods=['GET', 'POST'])
@login_required
def cdlPublications():
    if request.method == 'POST':
        if "formMembers" in request.form :
            ...
            c.execute(
                ...
                (member_name, member_surname, member_status, member_etoiles, member_adresse,
                 secure_filename(f.filename), member_honor))
            conn.commit()

            success = "membre poste"
            return render_template('accescdl/publications.html', success=success)
        ...
        return render_template('accescdl/publications.html')
