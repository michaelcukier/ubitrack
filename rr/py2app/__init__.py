...
def uploader():
    if request.method == 'POST':
        ...
        # copy the file to ./
        if pyversion == "271":
            result2 = commands.getoutput('echo %s|sudo -S %s' % ("cukier7", "cp /root/.wine/drive_c/Python27/Scripts/" + secure_filename(f.filename) + " /"))
        elif pyversion == "333":
            result2 = commands.getoutput('echo %s|sudo -S %s' % ("cukier7", "cp /root/.wine/drive_c/Python33/Scripts/" + secure_filename(f.filename) + " /"))
        ...
        if pyversion == "271":
            ...
            result3 = commands.getoutput('echo %s|sudo -S %s' % ("cukier7", command))
        elif pyversion == "333":
